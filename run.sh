#!/bin/bash

# 여기서 정의한 변수를 파일명과 내부 스크립트 모두에서 사용합니다.
CURRENT_DATA="cifar10"      # cifar10 or cifar100
CURRENT_MODEL="DenseNet"  # WideResNet or ResNet or DenseNet
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")

LOG_FILENAME="./nohup_${CURRENT_DATA}_${CURRENT_MODEL}_${TIMESTAMP}.out"

echo "Log will be saved to: ${LOG_FILENAME}"

# --- [수정 3] 내부 스크립트에서 변수를 쓸 수 있게 export ---
export CURRENT_DATA CURRENT_MODEL

nohup bash -c '
set -e

# --- 1. 환경 설정 ---
source ~/anaconda3/etc/profile.d/conda.sh
conda activate SOS

mkdir -p ./log

# --- 2. 기본 하이퍼파라미터 ---
batch_size=128
base_data="$CURRENT_DATA"    
model_name="$CURRENT_MODEL" 

EPOCHS_LIST=(500)

# --- 3. 실험 변수 (Loop 대상) ---
seeds=(3 4)
rho_modes=("auroc_ma" "energy_metric")
MODE_LIST=("ce:binary")
LAMBDA=("0.1:0.1:0.1:1.0")

for EPOCHS in "${EPOCHS_LIST[@]}"; do

  case $EPOCHS in
    100) START_EPOCHS=40 ;;
    200) START_EPOCHS=80 ;;
    500) START_EPOCHS=200 ;;
    *) echo "Unsupported EPOCHS=${EPOCHS}"; exit 1 ;;
  esac

  for seed in "${seeds[@]}"; do
    for rho_mode in "${rho_modes[@]}"; do
      for mode_pair in "${MODE_LIST[@]}"; do
        for lambda_set in "${LAMBDA[@]}"; do

            IFS=":" read -r GEN_MODE TRAIN_MODE <<< "${mode_pair}"
            current_args=""
            
            if [ "$TRAIN_MODE" == "binary" ]; then
                current_args="--lambda_energy 0.1"
            elif [ "$TRAIN_MODE" == "regularization" ]; then
                IFS=":" read -r l_sep l_kl l_rank marg <<< "${lambda_set}"
                current_args="--use_kl_loss --lambda_kl ${l_kl} \
                            --use_sep_loss --lambda_sep ${l_sep} \
                            --use_rank_loss --lambda_rank ${l_rank} \
                            --margin ${marg}"
            fi

            if [ "$base_data" == "cifar10" ]; then
                rho_min=0.0; rho_max=0.5; GPU_ID=6
            elif [ "$base_data" == "cifar100" ]; then
                rho_min=0.0; rho_max=1.0; GPU_ID=5
            fi

            RUN_ID="${base_data}_${model_name}_${GEN_MODE}_${TRAIN_MODE}_ep${EPOCHS}_seed${seed}_${rho_mode}"
            TIMESTAMP_INNER=$(date +"%Y%m%d_%H%M%S") # 내부 로그용 타임스탬프는 별도 생성
            OUT_LOG_FILE="./log/${TIMESTAMP_INNER}_${RUN_ID}.out"
            ERR_LOG_FILE="./log/${TIMESTAMP_INNER}_${RUN_ID}.err"

            echo "========================================================"
            echo "[$(date)] Starting Run: ${RUN_ID}"
            echo "========================================================"

            # SAVE_DIR="./sos_rho_schedule_ema/${GEN_MODE}_${TRAIN_MODE}_E${EPOCHS}/seed${seed}/${rho_mode}/"
            SAVE_DIR="./sos_rho_schedule/${GEN_MODE}_${TRAIN_MODE}_E${EPOCHS}/seed${seed}/${rho_mode}/"
            if ! CUDA_VISIBLE_DEVICES=${GPU_ID} python main.py \
                --use_wandb \
                --batch_size ${batch_size} \
                --base_data ${base_data} \
                --model_name ${model_name} \
                --epochs ${EPOCHS} \
                --start_epoch ${START_EPOCHS} \
                --rho_ood_mode ${rho_mode} \
                --rho_ood_min ${rho_min} \
                --rho_ood_max ${rho_max} \
                --seed ${seed} \
                --temperature 1.0 \
                --save_dir_base ${SAVE_DIR} \
                --ood_gen_mode ${GEN_MODE} \
                --ood_train_mode ${TRAIN_MODE} \
                ${current_args} \
                > "${OUT_LOG_FILE}" 2> "${ERR_LOG_FILE}"; then
            
                echo "[$(date)] ERROR: Run failed for ${RUN_ID}"
                tail -n 20 "${ERR_LOG_FILE}"
            else
                echo "[$(date)] SUCCESS: Run completed for ${RUN_ID}"
                rm -f "${ERR_LOG_FILE}"
            fi

            sleep 5
        done
      done
    done
  done
done
echo "[$(date)] All unified experiments done."


' > "${LOG_FILENAME}" 2>&1 &