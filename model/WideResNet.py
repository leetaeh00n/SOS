...
class WideResNet(nn.Module):
    ...
    def forward_virtual(self, x):
        ...
        out = self.relu(self.bn1(out))
        out = F.avg_pool2d(out, 8)
        penultimate = out.view(-1, self.nChannels)
    ...
    def forward_virtual_with_blocks(self, x):
        x = self.conv1(x)
        f1 = self.block1(x)
        f2 = self.block2(f1)
        f3 = self.block3(f2)
        f_out = self.relu(self.bn1(f3))  # final feature for classifier
        logits = self.fc(F.avg_pool2d(f_out, 8).view(f_out.size(0), -1))
        return logits, f_out, [f1, f2, f3]  # 마지막은 feature map list
